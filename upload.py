...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "https://inkinsights.vercel.app",   # your frontend
...
def _classical_mds(D: np.ndarray, dim: int = 2) -> np.ndarray:
    ...
    vals = vals[idx]
    vecs = vecs[:, idx]
    vals = vals[vals > 1e-9][:dim]
    vecs = vecs[:, :len(vals)]
    X = vecs @ np.diag(np.sqrt(vals)) if len(vals) else np.zeros((n, dim))
    if X.ndim == 1:
        X = X.reshape(-1, 1)
    if X.shape[1] < dim:
...
def find_themes(keywords, max_points=60):
    ...
    xs, ys = X[:, 0], X[:, 1]
    norm = lambda arr: (arr - np.min(arr)) / (np.max(arr) - np.min(arr) + 1e-9)
